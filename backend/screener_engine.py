...
class ScreenerEngine:
    ...
    def calculate_opening_rvol_dashboard(self, timeframe: str = "5m", days: int = 20, sort_order: str = "asc") -> List[Dict[str, Any]]:
        """
        Calculates 1st 5m/15m candle volume ratio for each stock compared against the 20-day 1st candle average.
        Returns stock results sorted in ascending (or descending) order.
        """
        universe = fyers_service.get_stock_universe()
        results = []
        ...
        for stock in universe:
            symbol = stock["symbol"]
            df = fyers_service.fetch_historical_candles(symbol, timeframe=tf, days=30)
            
            if df.empty or len(df) < 5:
                continue

            # Group candles by date YYYY-MM-DD
            if "date" in df.columns:
                df["day_str"] = pd.to_datetime(df["date"]).dt.strftime("%Y-%m-%d")
            else:
                df["day_str"] = pd.to_datetime(df["timestamp"], unit="s").dt.strftime("%Y-%m-%d")

            grouped = df.groupby("day_str", sort=False)
            
            first_candles = []
            for day, group in grouped:
                first_row = group.iloc[0]
                first_candles.append({
                    "day": day,
            ...
            if len(first_candles) < 2:
                continue

            # Today's 1st candle volume
            today_candle = first_candles[-1]
            today_vol = today_candle["volume"]

            # Past 20 days 1st candle volumes
            past_20 = first_candles[-min(days + 1, len(first_candles)):-1]
            if not past_20:
                past_20 = first_candles[-days:]

            past_vols = [c["volume"] for c in past_20]
            avg_20day_vol = sum(past_vols) / max(len(past_vols), 1)

            # Volume Ratio compared to 20-day 1st candle average
            rvol_ratio = round(today_vol / max(avg_20day_vol, 1.0), 2)
            rvol_pct = round(rvol_ratio * 100, 1)
        ...
        results.sort(key=lambda x: x["rvolRatio"], reverse=is_reverse)
